...
const mapLocations = [
  ...
  },
  {
    ...
    pin: { x: 35.8, y: 47 },
    wikiLinks: [
      { label: 'Doctor (medic trader)', to: '/wiki/npcs/doctor' },
      { label: 'School raster map', to: '/map/school-map' },
    ...
    npcLinks: [
      {
        ...
        role: 'Medic trader',
        blurb:
          'EA Build 1: School Build 1, 3rd floor—medical barter, lockers, trauma tasks. See the Doctor wiki for stairs and restock notes.',
        to: '/wiki/npcs/doctor',
      },
    ],
    content:
      'Playable School map in EA Build 1—transit from Village via the yellow-tape boundary. Doctor lives on the 3rd floor of Build 1; bandits can spawn on lower levels. Use /map/school-map for pins and the Doctor dossier for task tiers.',
    intel: {
      npcs: [],
      supplies: [
    ...
    npcLinks: [
      {
        label: 'Bandits',
        role: 'Hostiles',
        blurb: 'Standard spawns for legacy demo slices—see wiki for loot patterns if your build still ships Shipyard.',
        to: '/wiki/npcs/bandits',
      },
    ],
    wikiLinks: [
      { label: 'Doctor (now on School in EA)', to: '/wiki/npcs/doctor' },
      { label: 'Legacy Shipyard wiki archive', to: '/wiki/maps/shipyard' },
    ],
    content:
      'Poster pin for a demo-era industrial map—not in EA Build 1’s seven-map roster. Doctor moved to School; Gunsmith lives on Outpost. Keep the Shipyard wiki page for historical container tables if you still run an older demo branch.',
    intel: {
      npcs: [],
      supplies: [
    ...
    npcLinks: [
      {
        ...
        role: 'Weapons trader',
        blurb:
          'Outpost bunker behind the tank—weapon barter, plates, and attachments. Often tied to mid-run day events (~Day 10 in community notes).',
        to: '/wiki/npcs/gunsmith',
      },
      {
        label: 'Bandits',
        role: 'Area 05 hostiles',
        blurb: 'Open fields and military wreckage—pair with bandit loot notes when you farm Outpost routes.',
        to: '/wiki/npcs/bandits',
      },
    ],
    wikiLinks: [
      { label: 'Outpost raster map', to: '/map/outpost-map' },
      { label: 'Minefield (Border Zone)', to: '/wiki/maps/minefield' },
    ],
    content:
      'EA Build 1 Area 05 map—reach via Village → School → Highway transits. Gunsmith bunker sits behind a tank landmark; tent shelter nearby for saves. Official copy still frames the Border Zone (Minefield) as the next escalation east.',
    intel: {
      npcs: [
...
/** Plain href for static links / Vue islands without vue-router. */
export function mapHrefForPin(pinId) {
  const id =
    typeof pinId === 'string' && /^[a-z][a-z0-9-]*$/i.test(pinId) ? pinId.toLowerCase() : ''
  return id ? `${MAP_PAGE}?${MAP_PIN_QUERY}=${encodeURIComponent(id)}` : MAP_PAGE
}
