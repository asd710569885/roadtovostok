...
import mapLocations, { MAP_PIN_QUERY } from '../../data/maps.js'
import { RASTER_MAP_PAGES } from '../../data/raster-maps/rasterMapPages.js'
import { useUrlQuery } from './useUrlQuery.js'

const { value: pinQuery, setQuery: setPinQuery } = useUrlQuery(MAP_PIN_QUERY)
...
function applyRoutePinFocus() {
  const id = normalizePinQuery(pinQuery.value)
...
function clearSelection() {
  ...
  if (normalizePinQuery(pinQuery.value)) {
    setPinQuery('')
  }
}
      ...
      <div class="container">
        <div class="wrap wrap--full">
                        ...
                        <div v-if="selectedNpcLinks.length" class="map-detail__npcs">
                          ...
                          <ul class="map-detail__npc-list">
                            <li v-for="(n, idx) in selectedNpcLinks" :key="'npc-' + idx">
                              <a class="map-detail__npc-card" :href="n.to">
                        ...
                        >
                          ...
                          <ul class="map-detail__links">
                            <li v-for="(link, idx) in selectedWikiLinks" :key="'wl-' + idx">
                              <a class="map-detail__a" :href="link.to">{{ link.label }}</a>
                            </li>
                          </ul>
                        </nav>
          ...
          <p class="map-foot-links">
            <a class="map-link" href="/wiki">Wiki</a>
            ·
            <a class="map-link" href="/wiki/maps">Maps</a>
            ·
            <a class="map-link" href="/wiki/npcs">NPCs</a>
            ·
            <a class="map-link" href="/wiki/weapons">Weapons</a>
            ·
            <a class="map-link" href="/start">Start</a>
          </p>
        </div>
      </div>
    </section>
      ...
      <div class="container">
        ...
        <ul class="map-raster-tiles" role="list">
          <li v-for="m in RASTER_MAP_PAGES" :key="m.path" class="map-raster-tile-li">
            <a :href="m.path" class="map-raster-tile" :style="{ '--m-accent': m.accent }">
              <img
...
.map-board--has-focus .map-pin:not(.map-pin--active):is(:hover, :focus-visible) {
  opacity: 1;
  filter: none;
}
